...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
	...
	for (;;) {
		...
		case KVM_EXIT_IO:
			...
			//HC_print32bit is handled here
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
			    && vcpu->kvm_run->io.port == 0xE6) {
				unsigned int *p = (unsigned int *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset);
				printf("%u\n",*p);
				fflush(stdout);
				out_count++;
				continue;
			}
			//HC_printstr is handled here
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
			...
			//HC_gvatohva handled here
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
			    && vcpu->kvm_run->io.port == 0xF1) {
				unsigned int *k=(unsigned int *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset);
				
				trans.linear_address = *k;
				
				ioctl(vcpu->vcpu_fd, KVM_TRANSLATE, &trans);
			
				if(trans.valid==0){
					printf("Invalid GVA\n");
					trans.physical_address=0;
				}
				fflush(stdout);
				out_count++;
				continue;
			}
			//HC_numExitbyType handled here
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
			    && vcpu->kvm_run->io.port == 0xF5) {
				unsigned int  *p = (unsigned int *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset);
				out_count++;
				sprintf(vm->mem+*p, "IO in: %u\nIO out: %u\n", in_count, out_count);
				fflush(stdout);
				continue;
			}
			//HC_numExit IN is handled here
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN 
			    && vcpu->kvm_run->io.port == 0xE2) {
				unsigned int *q=(unsigned int *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset);
				*q=count;
				in_count++;
				continue;
			}
			//HC_gvaTohva IN is handled here
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN 
			    && vcpu->kvm_run->io.port == 0xF2) {
				if(trans.valid==1)
				{
					unsigned int *q=(unsigned int *)((char *)vcpu->kvm_run + vcpu->kvm_run->io.data_offset);
					*q = (uint64_t)((uintptr_t)trans.physical_address + vm->mem);
				}
